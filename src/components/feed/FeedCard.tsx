...
import { useBookmarks } from '../../pages/BookmarksPage';
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start gap-3 mb-4">
        <div className="flex gap-3 items-center min-w-0">
          <img src={authorAvatar} alt={authorName}
            className={`w-11 h-11 shrink-0 border-2 border-white shadow-soft ${post.authorRole === 'creator' ? 'rounded-full' : 'rounded-xl'}`} />
          <div className="min-w-0">
            <div className="flex items-center gap-1.5">
              <Link to={post.authorRole === 'creator' ? `/creator/${post.authorId}` : `/brand/${post.authorId}`}
            ...
            <p className="text-xs text-[#6E6A65] truncate max-w-[200px] sm:max-w-[260px] mt-0.5">{authorSub}</p>
        ...
        {post.collaborationMatchScore != null && (
          <div className="shrink-0">
            <ScoreBadge score={post.collaborationMatchScore} label={isCampaign ? 'Match' : 'Score'} size="sm" />
          </div>
        )}
      </div>

      <h3 className="text-base font-bold text-[#1F1F1F] mb-2 hover:text-[#A8678A] transition-colors leading-snug">{post.title}</h3>
      <p className="text-sm text-[#6E6A65] leading-relaxed mb-4 line-clamp-3 flex-1">{post.body}</p>

      {(post as FeedPost & { imageFilename?: string }).imageFilename && (
        <div className="mb-4">
          <img
            src={new URL(`../../assets/${(post as FeedPost & { imageFilename?: string }).imageFilename}`, import.meta.url).href}
            alt={post.title}
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        <span className={`px-2.5 py-0.5 rounded-full text-[11px] font-semibold capitalize ${catColor}`}>{post.category}</span>
      ...
      <div className="border-t border-[#E7E1D8] pt-3 flex items-center justify-between gap-2">
        ...
        <button type="button" onClick={handleSave}
          ...
          {saveFlash ? 'Saved! ✓' : saved ? 'Saved' : 'Save'}
        </button>
        ...
        <div className="flex gap-2">
          ...
          {!isCampaign && onContact && (
            <button type="button" onClick={() => onContact(post)}
